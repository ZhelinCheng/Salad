...
class Security {
    ...
    main(obj) {
        ...
        return this.isSize(obj.par.w, obj.par.h) ||
            this.isBrowser(obj.req.headers['user-agent'], obj.setting.userAgent) ||
            this.loadavg() ||
    ...
    isBrowser(ua, re) {
        ...
        let allowRe = new RegExp(re["allow"], 'i'),
            prohibitRe = new RegExp(re["prohibit"], 'i');
    ...
    isParameter(query, par, url, source) {
        ...
        if ((number.test(query.w) || number.test(query.h) || (number.test(query.o) && query.o)) && !state) {
            //参数不规范，没冗余参数
            state = urlArrangement();
        }
